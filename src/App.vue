<template>
    ...
    <nav class="container px-6 mx-auto md:flex md:justify-between md:items-center">
      <div class="flex items-center justify-between">
        <div class="relative w-16 h-16">
          <img class="rounded-full border border-gray-100 shadow-sm" :src="'./dist/assets/nrd.jpg'" alt="user image" />
        </div>
        <!-- Mobile menu button -->
    ...
    <div class="flex items-center">
      <div class="w-11/12 space-y-5 md:space-y-10">
        ...
        <h1
          ...
          It's
          <a href="https://www.linkedin.com/in/noureddine-eddallal-03aba3182/" target="_blank"
            class="text-black hover:underline">
            Noureddine Eddallal</a>🙋‍♂️<br />{{ findSlug('description').metadata.profession }}
        </h1>

        <div class="md:flex gap-1 md:gap-10">
          <button class="text-white bg-gray-800 submit-button hidden md:block">
            <a href="#work" v-smooth-scroll>View My Work</a>
          </button>

          <a class="underline text-2xl text-blue-600 font-work_sans" href="mailto:eddallal.noureddine@gmail.com"><button
              class="text-gray-800 border-4 border-gray-800 submit-button mb-5 md:mb-0">Contact Me</button></a>
      ...
      <div class="w-8/12 flex justify-end">
        <img class="3xl:h-[800px] 2xl:h-[650px] xl:h-[550px] lg:h-[550px] md:h-[450px] sm:h-[350px]"
          :src="'./dist/assets/man-1.png'" alt="user image" />
      </div>
    </div>
  </div>
  ...
  <div class="banner-2 space-y-10 pb-20" id="work">
    ...
    <div class="card" v-for="(post, index) in findSlug('projects').metadata.items" :key="index">
      <div class="space-y-5 py-8 px-8 md:py-16 md:px-20 md:w-1/2">
        <h4 class="project-title item">{{ post.title }}</h4>
        ...
        <ul class="flex">
          ...
          <li v-for="tech in post.techs" :key="tech"><img :src="'./dist/assets/icons/' + getTechIcon(tech).tech"
              style="width:45px" :title="getTechIcon(tech).name" /></li>
        </ul>
        <button class="text-sky-800 font-bold text-2xl tracking-wider">
          <a :href="post.link" target="_blank">View on Github</a>
        </button>
      </div>
  ...
  <div class="banner-2 md:flex items-center md:justify-evenly" id="about">
    <img class="md:h-[500px] h-[400px]" :src="'./dist/assets/man-2.png'" alt="user image" />
    <div class="space-y-5 py-8 px-8 md:py-16 md:px-20 md:w-1/2">
      <h4 class="project-title item">Sheepify States</h4>
      <p class="font-work_sans">{{ findSlug('description').metadata.pres_first }}</p>
      <p class="font-work_sans">{{ findSlug('description').metadata.pres_second }}</p>
      <p class="project-title item">Technologies</p>
      <div class="container mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('javascript').tech"
              style="width:50px" :title="getTechIcon('javascript').name" />
          </div>
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('vuejs').tech"
              style="width:50px" :title="getTechIcon('vuejs').name" />
          </div>
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('css').tech"
              style="width:50px" :title="getTechIcon('css').name" />
          </div>
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('bootstrap').tech"
              style="width:50px" :title="getTechIcon('bootstrap').name" />
          </div>

          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('php').tech"
              style="width:50px" :title="getTechIcon('php').name" />
          </div>
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('laravel').tech"
              style="width:50px" :title="getTechIcon('laravel').name" />
          </div>
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('html').tech"
              style="width:50px" :title="getTechIcon('html').name" />
          </div>
          <div class="flex justify-center text-6xl rounded-xl p-6">
            <img :src="'./dist/assets/icons/' + getTechIcon('wordpress').tech"
  ...
  <div class="banner-1 flex h-full items-center">
    <div class="w-7/12 p-12">
      <h2 class="text-gray-700 md:text-6xl text-2xl font-Eczar mb-5 font-bold">
        Let’s work together and make everything super simple and super useful.
      </h2>
      <a class="underline text-2xl text-blue-600 font-work_sans" href="mailto:eddallal.noureddine@gmail.com">📧
        eddallal.noureddine@gmail.com</a>
    </div>
    <div class="w-5/12 pr-28">
      <div class="flex flex-wrap justify-end gap-2">
        <a href="https://www.facebook.com/nordin.eddallal/" target="_blank">
        ...
        <a href="https://twitter.com/EddallalNordin" target="_blank">
          <button class="bg-gray-700 p-2 font-semibold text-white inline-flex items-center space-x-2 rounded">
            <svg class="w-4 h-4 md:w-10 md:h-10 lg:w-10 lg:h-10 xl:h-10 xl:w-10 fill-current" role="img"
              xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
        ...
        <a href="https://github.com/Alucard17th" target="_blank">
          <button class="bg-gray-700 p-2 font-semibold text-white inline-flex items-center space-x-2 rounded">
            <svg class="w-4 h-4 md:w-10 md:h-10 lg:w-10 lg:h-10 xl:h-10 xl:w-10 fill-current" role="img"
...
export default {
  ...
  methods: {
    ...
    getTechIcon(tech) {
      let techname = tech.charAt(0).toUpperCase() + tech.slice(1);
      // switch (tech) {
      //   case 'vuejs':
      //     // code block
      //     maman = "vuejs.png"
      ...
      //   case 'laravel':
      //     // code block
      //     maman = "laravel.248x256.png"
      //     break;
      //   case 'wordpress':
      //     // code block
      //     maman = "wordpress-logo-29018.png"
      //     break;
      //   default:
      //   // code block
      // }
      return { name: techname, tech: tech + ".png" }
    }
  },
  created() {
    document.body.classList.add("loading");
    Promise.all([this.fetchPosts(), this.fetchUser()]).then(([posts, user_data]) => {
      // user_data = this.n(user_data);
